...
public class OmeroN5FloatUtils extends OmeroN5Utils {
    ...
    public <T extends Type<T>> Img<T> getTileImage(
        ...
        RandomAccessibleInterval<FloatType> source = N5Utils.open(n5Reader, datasetPath);
        RandomAccess<FloatType> r = source.randomAccess();
        ...
        ArrayImg<FloatType, FloatArray> img = ArrayImgs.floats(region.width, region.height);
        RandomAccess<FloatType> dest = img.randomAccess();

        for(int i = 0; i < region.width; i++) {
            for(int j = 0; j < region.height; j++) {
                FloatType sampler = r.get();
                dest.get().set(sampler.getRealFloat());
            }
        }
        return (Img<T>) img;
      }
