...
public class OmeroN5UnsignedIntUtils extends OmeroN5Utils {
    ...
    public <T extends Type<T>> Img<T> getTileImage(
        ...
        RandomAccessibleInterval<UnsignedIntType> source = N5Utils.open(n5Reader, datasetPath);
        RandomAccess<UnsignedIntType> r = source.randomAccess();
        ...
        ArrayImg<UnsignedIntType, IntArray> img = ArrayImgs.unsignedInts(region.width, region.height);
        RandomAccess<UnsignedIntType> dest = img.randomAccess();

        for(int i = 0; i < region.width; i++) {
            for(int j = 0; j < region.height; j++) {
                UnsignedIntType sampler = r.get();
